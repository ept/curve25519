...
Strictly speaking, this function allows the value to be within the range $[0,\, 2^{255} - 1]$, including the values $\{2^{255} - 19,\, \dots,\, 2^{255} - 1\}$ that are not reduced modulo $p$, but this not do any harm, since the functions that take the \verb|field_elem| type as input handle these numbers correctly.
...
The Montgomery ladder~\cite{Bernstein:2017fm,Montgomery:1987fz} is the algorithm used by X25519 to efficiently perform scalar multiplication.
