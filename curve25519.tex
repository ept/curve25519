...
\paragraph{Handling the point at infinity.}
...
\begin{itemize}
    \item Let $Z_i=0$ and $X_i \ne 0$.
        Then $Z_{2i}=0$ and $X_{2i} = X_i^4 \ne 0$, which matches the behaviour $\infty\bullet\infty=\infty$ required by~\eqref{eq:law-identity}.
    ...
    \item Let $Z_i \ne 0$, $Z_{i+1} \ne 0$, and $X_i/Z_i = X_{i+1}/Z_{i+1}$.
        ...
        Finally, we need to show that $X_{2i+1} \ne 0$.
        ...
        \begin{enumerate}
            \item If $X_i + Z_i = 0$ then $X_i = -Z_i$ so $X_i/Z_i = -1$.
                By assumption $X_i/Z_i = X_{i+1}/Z_{i+1}$ we have $X_{i+1}/Z_{i+1} = -1$.
                TODO need to derive a contradiction.
            \item If $X_i + Z_i \ne 0$ then $X_{i+1} = Z_{i+1}$ so $X_{i+1}/Z_{i+1} = 1$.
                By assumption $X_i/Z_i = X_{i+1}/Z_{i+1}$ we have $X_i/Z_i = 1$.
                TODO need to derive a contradiction.
        \end{enumerate}
    \item Let $Z_i=0$ and $X_i \ne 0$, so $P_i = \infty$.
    ...
    \item Let $Z_{i+1} = 0$ and $X_{i+1} \ne 0$, so $P_{i+1} = \infty$.
        Since $P_{i+1} = P_i \bullet P$ and $P \ne \infty$ we have $P_i = P^{-1}$.
        TODO finish this.
\end{itemize}

\paragraph{The optimised Montgomery ladder step.}

Further manipulation of the $X_{2i}$ coordinate from~\eqref{eq:projective-double}:
\begin{align*}
X_{2i} &= (X^2 - Z^2)^2 \\
&= X^4 - 2X^2 Z^2 + Z^4 \\
...
&= (X + Z)^2\,(X - Z)^2
\end{align*}
TODO: $Z$ coordinate expression isn't in the right form yet.
