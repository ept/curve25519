...
\verb|t| is a 32-element array of 64-bit integers, initialised to zero.
...
\begin{align*}
    a \cdot b &= (a_0 2^0 + a_1 2^{16} + a_2 2^{32} + \dots + a_{15} 2^{240}) \cdot (b_0 2^0 + b_1 2^{16} + b_2 2^{32} + \dots + b_{15} 2^{240}) \\
    &= a_0 b_0 2^{0+0} + a_1 b_0 2^{16+0} + \dots + a_{15} b_0 2^{240+0} + a_0 b_1 2^{0+16} + a_1 b_1 2^{16+16} + \dots + a_{15} b_{15} 2^{240+240} \\
    &= a_0 b_0 2^0 + (a_1 b_0 + a_0 b_1)\, 2^{16} + (a_2 b_0 + a_1 b_1 + a_0 b_2)\, 2^{32} + \dots + a_{15} b_{15} 2^{480}
\end{align*}
The 32-element array \verb|t| now contains the product of \verb|a| and \verb|b| (the product of two 255-bit numbers is a 510-bit number).
Only elements 0 to 30 are actually needed, because the last element is always zero.

In order to avoid the numbers getting bigger and bigger, this is a good time to reduce modulo $p$, as explained in \autoref{sec:modular-arithmetic}.
...
\begin{align*}
    ...
    &= (t_0 + 38\, t_{16})\, 2^0 + (t_1 + 38\, t_{17})\, 2^{16} + \dots + (t_{14} + 38\, t_{30})\, 2^{224} + t_{15} 2^{240}
\end{align*}
The reduction is ``almost'' because, although the number now fits in the \verb|gf| type with 16 elements, each individual element is likely to fall outside of the $[0,\, 2^{16}-1]$ range, so the number as a whole is not fully reduced modulo $2p$.
In order to normalise the values of these elements, the \verb|M| function first copies the product into the output variable \verb|o|, and then calls the \verb|car25519| function twice (lines~44--46).

The \verb|car25519| function (lines~14--24 of \autoref{code:arithmetic}) cleans up a value of type \verb|gf|.
...
Returning an error instead of the point at infinity would make the function non-constant-time, if not done carefully.
...
In the course of Diffie-Hellman with a base point of order $q$, $\infty$ will never be generated, since $jk$ can never be a multiple of $q$ in $(P^j)^k = P^{jk}$.
