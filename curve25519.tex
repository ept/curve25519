...
We do this by carefully studying one particular algorithm, the X25519 Diffie-Hellman key agreement scheme, and its implementation in the TweetNaCl library.
...
Starting from first principles, this document shows how to re-derive the TweetNaCl implementation of X25519, including all of the tedious algebra, fully explaining and justifying every line of code.
...
Glancing at an implementation of X25519 (see, for example, \autoref{code:scalarmult} on page~\pageref{code:scalarmult}) reveals mysterious sequences of arithmetic operations with few comments, explanation, or justification.
...
If the adversary may actively interfere with the communication, additional authentication is required, which we don't discuss here.
...
The parameters of Curve25519 are specifically chosen such that the order of the group is the product of a small integer $h$ (which is called the \emph{cofactor}, in this case 8), and a prime that is slightly greater than a large power of 2.
...
sv A(gf o,const gf a,const gf b) /* add: o = a + b */
{
  int i;
  FOR(i,16) o[i]=a[i]+b[i];
}

sv Z(gf o,const gf a,const gf b) /* subtract: o = a - b */
{
  int i;
  FOR(i,16) o[i]=a[i]-b[i];
}

sv M(gf o,const gf a,const gf b) /* multiply: o = a * b */
{
  i64 i,j,t[31];
  FOR(i,31) t[i]=0;
...
sv S(gf o,const gf a) /* square: o = a^2 */
{
  M(o,a,a);
}
\end{minted}
...
In the last line, it forces the most significant bit (the 255th bit) to be zero, since our numbers are always less than $2^{255}$.
Strictly speaking, this function allows the value to be within the range $[0,\, 2^{255} - 1]$, including the values $\{2^{255} - 19,\, \dots,\, 2^{255} - 1\}$ that are not reduced modulo $p$, but this not do any harm, since the functions that take the \verb|gf| type as input handle these numbers correctly.
...
% Here's an example value that requires three calls to car25519(val) before all elements are within [0, 0xffff]:
...
Consider first the case where $y_P=-y_Q \ne 0$, which is shown in \autoref{fig:vertical}: if we draw a straight line through the two points, that line is vertical, and there is no third intersection point.
We will return to this case, and the $y_P=y_Q=0$ case, in the next section.
...
To compute the slope $\lambda$ of this tangent, we can calculate the derivative of the curve equation~\eqref{eq:curve}:
\begin{align}
...
If $y=0$, the tangent is vertical, and we handle this as part of the case $y_P=-y_Q$ in the next section.
...
We won't discuss this cipher any further in this document, as it is beyond the scope of X25519.
...
Thus, each loop iteration maps the input tuple $(X_i, Z_i, X_{i+1}, Z_{i+1})$ to either the output tuple $(X_{2i}, Z_{2i}, 4X_{2i+1}, 4Z_{2i+1})$ or the output tuple $(4X_{2i+1}, 4Z_{2i+1}, X_{2i+2}, Z_{2i+2})$ depending on the value of \verb|r|.
...
The TweetNaCl implementation would be constant-time even without setting this bit to 1, but X25519 is defined to always have this bit set as a precaution to protect less careful implementations.
...
% PAKE, such as https://github.com/jedisct1/cpace or socialist millionaire protocol
...
\begin{acks}
Thank you to Daniel Hugenroth for feedback on a draft of this document.
