...
The function $L(P, i)$ takes a group element $P$ and a non-negative integer $i$, and it returns a pair of group elements $(P^i, P^{i+1})$ computed using the group operator $\bullet$:
\begin{align}
L(P,\, 0) &= (\infty, P) \\
...
The second argument is halved (rounding down) on each recursive call, resulting in $\lfloor\log_2 i\rfloor + 1$ recursive calls to compute $L(P, i)$ (i.e.\ one call per bit of $i$).
...
(\autoref{code:scalarmult} on page~\pageref{code:scalarmult} shows the TweetNaCl implementation of the Montgomery ladder; lines~19 to~43 contain the main loop that iterates over the bits of the integer.)
...
\paragraph{Optimised formulas for point doubling.}
...
Using the expression we derived in~\eqref{eq:law-x}:
\begingroup
\allowdisplaybreaks
\begin{align*}
x_{2i} &= \frac{(3x^2 + 2ax + 1)^2}{4y^2} - a - 2x \\[5pt]
&= \frac{(3x^2 + 2ax + 1)^2}{4\,(x^3 + ax^2 + x)} - a - 2x 
    \quad\text{ since } (x, y) \text { is on the curve } y^2 = x^3 + ax^2 + x \\[5pt]
&= \frac{(3x^2 + 2ax + 1)^2 - 4\,(x^3 + ax^2 + x)(2x + a)}{4\,(x^3 + ax^2 + x)} \\[5pt]
&= \frac{9x^4 + 12ax^3 + (4a^2 + 6)\,x^2 + 4ax + 1 -4\,(2x^4 + 3ax^3 + (a^2 + 2)\,x^2 + ax)}{4\,(x^3 + ax^2 + x)} \\[5pt]
&= \frac{x^4 -2x^2 + 1}{4\,(x^3 + ax^2 + x)} 
    \;=\; \frac{\frac{X^4}{Z^4} - \frac{2X^2}{Z^2} + 1}{4\,\left(\frac{X^3}{Z^3} + \frac{aX^2}{Z^2} + \frac{X}{Z}\right)}
    \quad\text{ using projective coordinates } x=\frac{X}{Z}\\[5pt]
&= \frac{\frac{1}{Z^4}\,(X^4 - 2X^2 Z^2 + Z^4)}{\frac{4}{Z^3}\,(X^3 + aX^2 Z + X Z^2)}
    \;=\; \frac{X^4 - 2X^2 Z^2 + Z^4}{4Z\,(X^3 + aX^2 Z + XZ^2)} \\[5pt]
&= \frac{X^4 + 2X^3 Z + X^2 Z^2 - 2X^3 Z - 4 X^2 Z^2 - 2X Z^3 + X^2 Z^2 + 2X Z^3 + Z^4}{4Z\,(X^3 + aX^2 Z + XZ^2)} \\[5pt]
&= \frac{(X^2 + 2XZ + Z^2)\,(X^2 - 2XZ + Z^2)}{4Z\,(X^3 + aX^2 Z + XZ^2)} \\[5pt]
&= \frac{(X + Z)^2\,(X - Z)^2}{4Z\,(X^3 + aX^2 Z + XZ^2)}
\end{align*}
\endgroup
...
\left(\frac{X_{2i}}{Z_{2i}}, \frac{Y_{2i}}{Z_{2i}}\right) \quad\text{where}\quad
    \begin{array}{rl}
        X_{2i} &= (X_i + Z_i)^2\,(X_i - Z_i)^2 \\[5pt]
        Z_{2i} &= 4Z_i\,(X_i^3 + aX_i^2 Z_i + X_i Z_i^2)
    \end{array}
\end{equation}
...
\paragraph{Optimised formulas for point addition.}
